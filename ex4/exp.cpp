...
Expression* Let::eval(Env& en){
	...
	if (value_evaluated->isFunction())	//если это была функция
		value_evaluated->func_setEnv(en);	//то оно меняет её, прицепляя ей текущий environment
		/*при этом у функции в environment'е
		станет определена переменная, соответствующая этой самой функции
		(т. ч. рекурсия будет работать), -
		потому что указатель на неё в её environment'е
